...
@pytest.mark.asyncio
async def test_restaurant_a_graph_greets_on_first_turn(monkeypatch):
    fake_response = MagicMock()
    fake_response.choices[0].message.content = "Welcome!"
    ...
    result = await restaurant_a_graph.ainvoke(state)

    assert result["turn_count"] == 1
    assert result["messages"][-1]["content"] == "Welcome!"